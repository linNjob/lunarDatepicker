...
// 依序塞入 農曆
function funLunarData() {
  $('.lunarList li span').empty();
  $('.lunarList li').each(function (i, item) {
    const itemContent = $(item).find("span");
    let lunar = calendar.solar2lunar(itemContent.data('year'), itemContent.data('month'), itemContent.data('day'))
    lunar.IDayCn == '初一' ? 
      itemContent.addClass('first').append(lunar.IMonthCn) : 
      itemContent.append(lunar.IDayCn);
  })
}
